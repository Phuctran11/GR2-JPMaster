...
import adminModel, { AdminJlptSectionInput, BlogStatus, JlptLevel, LessonContentType, QuestionType, QuizType, SectionType, SortOrder, UserRole, UserStatus } from "../models/admin.model.js";
import userModel from "../models/user.model.js";
import passwordService from "../services/password.service.js";
import { AuthenticatedRequest } from "../middlewares/auth.middleware.js";

const USER_ROLES: UserRole[] = ["guest", "learner", "admin"];
const USER_STATUSES: UserStatus[] = ["active", "suspended"];
const COURSE_LEVELS = ["beginner", "intermediate", "advanced"] as const;
const LESSON_TYPES: LessonContentType[] = ["video", "text", "quiz"];
...
export class AdminController {
  ...
  async createCourse(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.createCourse({
        ...
        created_by: Number(req.body.created_by) || req.user!.user_id,
      });
  ...
  async deleteCourse(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const deleted = await adminModel.deleteCourse(courseId);
  ...
  async createLesson(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const contentType = req.body.content_type as LessonContentType;
      const orderIndex = Number(req.body.order_index ?? 1);

      if (!title || !Number.isFinite(courseId) || !LESSON_TYPES.includes(contentType) || !Number.isFinite(orderIndex)) {
        return res.status(400).json({ error: "course_id, title, content_type, and order_index are required" });
      }
  ...
  async updateLesson(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (req.body.content_type !== undefined && !LESSON_TYPES.includes(req.body.content_type)) {
        return res.status(400).json({ error: "valid content_type is required" });
      }
  ...
  async deleteLesson(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const deleted = await adminModel.deleteLesson(lessonId);
  ...
  async deleteQuiz(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const deleted = await adminModel.deleteQuiz(quizId);
  ...
  async listQuizQuestions(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.listQuizQuestions(quizId);
      return res.status(200).json({ data, count: data.length });
    } catch (error) {
      next(error);
    }
  ...
  async createQuizQuestion(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.createQuizQuestion(quizId, parsed.data, req.user!.user_id);
  ...
  async updateQuizQuestion(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.updateQuizQuestion(quizId, questionId, parsed.data);
  ...
  async deleteQuizQuestion(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const deleted = await adminModel.deleteQuizQuestion(quizId, questionId);
  ...
  async updateQuizQuestionOrder(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const updated = await adminModel.updateQuizQuestionOrder(quizId, questionId, orderIndex);
  ...
  async deleteJlptExam(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const deleted = await adminModel.deleteJlptExam(examId);
  ...
  async listJlptSections(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.listJlptSections(examId);
      return res.status(200).json({ data, count: data.length });
    } catch (error) {
      next(error);
    }
  ...
  async deleteJlptSection(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const deleted = await adminModel.deleteJlptSection(sectionId);
  ...
  async listJlptSectionQuestions(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.listJlptSectionQuestions(sectionId);
      return res.status(200).json({ data, count: data.length });
    } catch (error) {
      next(error);
    }
  ...
  async createJlptSectionQuestion(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.createJlptSectionQuestion(sectionId, parsed.data, req.user!.user_id);
  ...
  async updateJlptSectionQuestion(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const data = await adminModel.updateJlptSectionQuestion(sectionId, questionId, parsed.data);
  ...
  async updateJlptSectionQuestionOrder(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const updated = await adminModel.updateJlptSectionQuestionOrder(sectionId, questionId, orderIndex);
  ...
  async deleteJlptSectionQuestion(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const deleted = await adminModel.deleteJlptSectionQuestion(sectionId, questionId);
